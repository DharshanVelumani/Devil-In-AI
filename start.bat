...
echo.
echo ---------------------------------------------
echo    Devil-In AI - Zero Dependency
echo    Windows x64
echo ---------------------------------------------
echo.
...
if %MODEL_COUNT% equ 1 (
    ...
    for /l %%i in (1,1,%MODEL_COUNT%) do (
        for %%m in ("!MODEL_%%i!") do (
            set "NAME=%%~nxm"
            for /f "usebackq delims=" %%s in (`powershell -NoProfile -Command "[math]::Round((Get-Item '%%~ffm').Length / 1MB)"`) do set "SIZE_MB=%%s"
        )
...
if not exist "%BIN%" (
    if exist "%BIN_DIR%\llama-server.exe" (
        copy /y "%BIN_DIR%\llama-server.exe" "%BIN%" >nul
    )
)
...
:: Prepend our DLL dir so our DLLs always win over anything in PATH
...
echo [+] Theme    : Premium Red ^& Gold Edition
echo.
echo Press Ctrl+C to stop the server.
echo ------------------------------------

:: ---------- Open browser when server is ready --------------------------------
start "" /b powershell -NoProfile -Command "do { Start-Sleep -Seconds 1; try { if ((Invoke-WebRequest -Uri 'http://127.0.0.1:8080/v1/models' -UseBasicParsing -ErrorAction Stop).StatusCode -eq 200) { break } } catch {} } while ($true); start http://127.0.0.1:8080"
